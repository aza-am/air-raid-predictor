...
**Примітка щодо структури**: "build_features.py" знаходиться в папці **preprocessing/** та "collect_data.py" відповідно в **data_collection/** відповідно до їхнього функціонального призначення. У run_pipeline.py шлях до нього може відрізнятись залежно від розташування файлу безпосередньо у вашому проєкті (радимо об'єднати це в папку, наприклад, **collecting/**, де збір і підготовка вхідних даних(зборів ISW та Telegram) для моделі йтимуть разом).
...
**forecast_update.py** завантажує нашу натреновану модель з регіональними порогами класифікації, будує вхідні дані з latest_input.csv та генерує прогноз тривог для всіх регіонів на 24 години, зберігаючи результат у data/forecast.json
