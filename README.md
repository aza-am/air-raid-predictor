...
**data_collection/** збирає все необхідне для моделі за один запуск: робить запити до Visual Crossing API для отримання погоди, скрапить звіти ISW через BeautifulSoup та витягує останні пости з Telegram через Telethon.

**preprocessing/** перетворює сирі дані на ознаки для моделі: розраховує часові лаги (24, 48, 72, 168 годин), створює бінарні ознаки (ніч/вихідні) та векторизує тексти звітів через TF-IDF.

**run_pipeline.py** — скрипт, що координує весь pipeline від збору даних до збереження прогнозу для AWS Cron. По черзі запускає збір даних, передобробку та фінальний прогноз, зберігаючи результат у JSON.
